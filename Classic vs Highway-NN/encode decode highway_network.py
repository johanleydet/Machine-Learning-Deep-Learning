...
class H(nn.Module):
    def __init__(self,size):
        ...
        self.T = nn.Linear(size,1)
    
    def forward(self,x):
        t = F.sigmoid(self.H(x))
        x = F.relu(self.H(x))*t + x*(1-t)
        return x

class highway(nn.Module):
    def __init__(self,nb_layers, size):
        super(highway, self).__init__()
        self.nb_layers=nb_layers
        self.layers=H(size).double()
...
class autoEncodeur(nn.Module):
    ...
    def forward(self,X):
        ...
        return self.decode(self.H(self.encode(X)))
...
# State #########################################################
#################################################################
savepath = Path("TME3_highway_network_batch_50.pch")
...
#################################################################
plt.figure(1)
plt.plot(state.test_loss, label='train highway batch 50') 
plt.plot(state.train_loss, label='test highway batch 50') 
...
plt.title('Encode Decode Image by highway')
plt.figure()
